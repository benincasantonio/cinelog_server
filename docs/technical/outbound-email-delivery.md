...
| `lock_token` | Per-claim fencing token minted on claim, required by every settlement, cleared whenever the lock ends |
...
2. `UPDATE ... WHERE id IN (...)` setting `status = 'processing'`, `locked_at = now()`,
   `lock_token = gen_random_uuid()`, `attempt_count = attempt_count + 1`.
3. Commit.
...
`FOR UPDATE SKIP LOCKED` prevents two workers claiming the same row *at the same moment*,
but it says nothing after the claiming transaction commits. `status` and `locked_at`
represent the lock from then on, and a worker that hangs past `lock_timeout` wakes up
believing it still owns a row that has since been requeued and re-claimed:

1. A claims the message — `attempt_count = 1`, `status = 'processing'`.
2. A hangs.
3. Five minutes later the stale sweep returns the row to `pending`.
4. B claims it — `attempt_count = 2`.
5. A resumes. Both A and B now believe they own the row.

Every claim therefore mints a fresh `lock_token` (`gen_random_uuid()`, generated by
PostgreSQL so each row in a batch gets its own), returns it to the worker, and requires
...
The token is cleared wherever a lock ends — settlement, release, stale recovery,
supersede, expiry — so a superseded worker matches zero rows and `mark_delivered()`,
`schedule_retry()`, `mark_failed()`, and `release_claims()` all return `False`/`0`, which
the delivery service logs as a lost lease.

Without the fence this sequence corrupts a row: A stalls, the sweep requeues it, B claims
and delivers it and clears its bodies, then A reports failure and regresses the delivered
row to `pending` — now with no rendered content, so every later attempt fails until it
exhausts.

A random token is used rather than `attempt_count` because the counter is not unique over
time: `release_claims()` refunds an unspent attempt, and an operator resetting a stuck
queue by hand (`SET status = 'pending', attempt_count = 0`) can recreate a
`(status, attempt_count)` pair an old worker still holds. A UUID cannot be reproduced by
either.
...
attempt, fenced on the lock token so a row already re-claimed elsewhere is skipped.
